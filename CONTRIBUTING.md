...
1. Run `make dev-setup` once on your machine so you have the required tools used by CI and local helper targets (SwiftLint, XcodeGen, `cloc`). ImageMagick and `diff-image` are optional helpers for visually diffing snapshot PNGs.
